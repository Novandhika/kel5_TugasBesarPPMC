...
void fillArray(char **arr, FILE *f)
{
    ...
    char ch;
    while (((ch = fgetc(f)) != '#')&&(count_row < rows_game+2)) {
        if (ch != '\n') {
...
void nextGeneration(char **arr, char **future)
{
    ...
    for (i = 0; i < rows_game; i++)
    {
        for (j = 0; j < cols_game; j++)
        {
            if((arr[i][j] == 'X')&&((cellNeighbors(arr,i,j)-1) < 2)){	//di -1 karena proses cellNeighbors menghitung dirinya sendiri(tidak hanya disekelilingnya)
                future[i][j] = '-';
...
//Apakah ini dipakai? masi ngambang euyy
...
void end_generation(char **arr, char **future)
{
    char temp;
    nextGeneration(arr, future);
    showArray(future);
    system("cls");
    delay(0.25);
    while (lanjut_iterasi(arr,future) == 0)
    {
        for(int b = 0; b<rows_game; b++){
            for(int k = 0; k<cols_game; k++){
                temp = arr[b][k];
